...
> **Status: verified on real hardware (confirmed firing on cursor-agent CLI 2026.07.08).** Cursor hooks send the event payload as stdin JSON containing `hook_event_name` (camelCase), `conversation_id`, and `workspace_roots` (an array), so no argv event name is needed (confirmed to match the real payload).

Merge the `hooks` object of `build/config/cursor-hooks.json` (generated by `setup.sh`) into `~/.cursor/hooks.json`. Both the Cursor IDE and the cursor-agent CLI read `~/.cursor/hooks.json` (CLI firing verified on real hardware).

- `beforeSubmitPrompt` / `preToolUse` / `postToolUse` / `afterFileEdit` → running (🟡)
- `beforeShellExecution` / `beforeMCPExecution` → waiting (🔴)
- `afterShellExecution` / `afterMCPExecution` → running (🟡)
- `stop` / `sessionStart` → idle (🟢)
- Note: confirmed on real hardware: `sessionStart` → idle, `preToolUse` → running, `beforeShellExecution` → waiting (the message includes "waiting for approval to run \<command\>"), `afterShellExecution` → running, `postToolUse` → running, status file removed when the session ends. `beforeSubmitPrompt` was not observed to fire in non-interactive mode (`-p`); interactive mode is untested
- Note: Cursor has no dedicated "waiting for approval" event, so `beforeShellExecution` / `beforeMCPExecution` firing is treated as waiting (🔴) instead. With auto-run (`--trust` / auto-run) the indicator flashes red before each command; because `beforeShellExecution` → `afterShellExecution` fire back-to-back, the red lasts only about 0.3–0.5s (measured) before returning to yellow. The adapter prints `{}` when invoked with `cursor` (no permission decision = default flow)

## GitHub Copilot CLI integration

> **Status: verified on real hardware (confirmed firing across the full lifecycle on Copilot CLI 1.0.69).** Because the event name field in the payload is not well documented, the adapter is invoked with an explicit argv event name (using the same names as Claude Code) instead of relying on payload contents.

Copy `build/config/copilot-hooks.json` (generated by `setup.sh`) to `~/.copilot/hooks/andon.json`. Copilot CLI loads hooks from `~/.copilot/hooks/*.json`. Every event below was confirmed firing on real hardware:

- `userPromptSubmitted` / `preToolUse` / `postToolUse` / `postToolUseFailure` → running (🟡)
- `permissionRequest` / `notification` → waiting (🔴)
- `sessionStart` / `agentStop` → idle (🟢)
- `sessionEnd` → removed from display (status file deleted)
- Note: for `notification` approval requests, the message includes "Run command: \<command\>"
- Note: `permissionRequest` fires even when the tool ends up auto-approved. Also, between approval and the tool actually finishing, the indicator stays red until the next event (`postToolUse`) arrives — Copilot CLI has no event marking "execution started." For long-running commands this can make the indicator look red for a while even after approval
- With `--allow-all-tools`, the indicator still flashes red briefly for each tool call
...
hooks/generic_status_hook.py    generic adapter for Codex / Antigravity / Cursor / Copilot CLI etc.
